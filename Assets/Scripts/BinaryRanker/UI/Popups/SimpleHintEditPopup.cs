...
public class SimpleHintEditPopup : SimpleEditPopup
{
    ...
    public void Show(string title, string currentValue, Action<string> onSave, Func<string, bool> onValidate, List<string> recommendedValues)
    {
        _RecommendedValuesPanel.SetData(recommendedValues);

        base.Show(title, currentValue, onSave, onValidate);
    }
